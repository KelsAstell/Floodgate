...
IGNORE_EMOTES = False                                      # 是否忽略纯表情消息（仅含 faceType 标签且无文字内容），开启后不会转发给 OneBot
SUBSCRIBED_MESSAGE_TYPES = ["GROUP_AT_MESSAGE_CREATE", "GROUP_MESSAGE_CREATE", "C2C_MESSAGE_CREATE", "AT_MESSAGE_CREATE"]  # 订阅的消息事件类型，可自由增删；这些类型的事件会被解析并转发给 OneBot 客户端
GM_BLACKLIST = []                                             # 全量消息(GROUP_MESSAGE_CREATE)黑名单，数字群ID列表；可使用 ~gm_blacklist 命令管理
