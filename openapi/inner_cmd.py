...
async def parse_floodgate_cmd(start_time,connected_clients,payload,headers): #直接传个d进来应该够用，我错了
    ...
    if cmd.startswith("health"):
        ...
        # 同时显示用户与当前群聊的 OpenID/数字ID（仅群聊可用）
        group_openid = d.get("group_openid") or d.get("channel_id")
        if not group_openid:
        ...
        if not TRANSPARENT_OPENID:
            ...
            user_digit_id = await get_or_create_digit_id(user_openid) if user_openid else None
            msg = f"当前用户信息：\nOpenID: {user_openid}\n数字ID: {user_digit_id}\n\n当前群聊信息：\nOpenID: {group_openid}\n数字ID: {group_digit_id}"
        else:
        ...
        if len(parts) <= 1 or parts[1].strip() == "":
            ...
            test_payload = {
                "content": "订阅功能测试成功！每天早 8:00 将自动推送消息。",
                "msg_type": 0,
            }
            try:
                result = await call_open_api(
                    "POST", f"/v2/groups/{group_openid}/messages",
                    test_payload, sleepy=False
                )
                if isinstance(result, dict) and result.get("send_failed"):
                    raise Exception("send_failed")
            except Exception as e:
                log.warning(f"主动消息测试失败，群: {group_digit_id}, 原因: {e}")
                return await post_floodgate_message(
                    "订阅功能测试失败，请在手机端点击机器人头像，"
                    "右上角设置中，打开「机器人主动在群聊内发言」开关后再试。",
                    d
                )
            # 测试通过，写入数据库
            ...
            if subscribed:
                from openapi.subscription import _get_subscription_expiry
                expiry = await _get_subscription_expiry(group_openid)
                try:
                    from datetime import datetime
                    exp_dt = datetime.strptime(str(expiry), "%Y-%m-%d %H:%M:%S")
